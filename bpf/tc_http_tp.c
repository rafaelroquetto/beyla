...
static __always_inline void encode_hex_skb(unsigned char *dst,
...
make_tp_string_skb(unsigned char *buf, const tp_info_t *tp, const unsigned char *end) {
    ...
    *buf++ = '-';
    ...
    // Trace ID
    encode_hex_skb(buf, tp->trace_id, TRACE_ID_SIZE_BYTES, end);
    buf += TRACE_ID_CHAR_LEN;

    *buf++ = '-';

    // SpanID
    encode_hex_skb(buf, tp->span_id, SPAN_ID_SIZE_BYTES, end);
